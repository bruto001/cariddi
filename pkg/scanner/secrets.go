	...
	onceSecret.Do(func() {
		secrets = []Secret{
			...
			},
			{
				"AWS Bedrock API Key",
				"AWS Bedrock API Key",
				*regexp.MustCompile(`ABSK[A-Za-z0-9+/]{109,269}={0,2}`),
				[]string{},
				"?",
			},
			{
			...
			},
			{
				"GitLab Feed Token",
				"GitLab Feed Token",
				*regexp.MustCompile(`glft-[0-9a-zA-Z_\-]{20}`),
				[]string{},
				"?",
			},
			{
			...
			{
				"GitLab OIDC Application Secret",
				"GitLab OIDC Application Secret",
			...
			},
			{
				...
				"GitLab Pipeline Trigger Token",
				*regexp.MustCompile(`glptt-[0-9a-f]{40}`),
				[]string{},
				"?",
			},
			{
			...
			},
			{
				"Heroku API Key v2",
				"Heroku API Key v2",
				*regexp.MustCompile(`(?i)HRKU-AA[0-9a-zA-Z_-]{58}`),
				[]string{},
				"?",
			},
			{
			...
			},
			{
				"OpenAI API key",
				"OpenAI API key",
				*regexp.MustCompile(`(sk-(?:proj|svcacct|admin)-(?:[A-Za-z0-9_-]{74}|[A-Za-z0-9_-]{58})` +
					`T3BlbkFJ(?:[A-Za-z0-9_-]{74}|[A-Za-z0-9_-]{58})\b|sk-[a-zA-Z0-9]{20}T3BlbkFJ[a-zA-Z0-9]{20})`),
				[]string{},
				"?",
			},
			{
			...
			},
			{
				"Perplexity API Key",
				"Perplexity API Key",
				*regexp.MustCompile(`pplx-[a-zA-Z0-9]{48}`),
				[]string{},
				"?",
			},
			{
			...
			},
			{
				"Postman API Key",
				"Postman API Key",
				*regexp.MustCompile(`PMAK-(?i)[a-f0-9]{24}\-[a-f0-9]{34}`),
				[]string{},
				"?",
			},
			{
				"Rubygems API Key",
				"Rubygems API Key",
				*regexp.MustCompile(`rubygems_[a-f0-9]{48}`),
				[]string{},
				"?",
			},
			{
			...
			},
			{
				...
				"Azure Blob/ADLS Storage",
				*regexp.MustCompile(`(?i)(?:https?:\/\/)?([a-z0-9]{3,24})\.(blob|dfs)\.core\.windows\.net`),
				[]string{},
				"?",
			},
			{
				...
				"Google Cloud Storage",
				*regexp.MustCompile(`(?i)(?:https?:\/\/storage\.googleapis\.com\/[a-z0-9.\-_]{3,63}\/[^\s"'<>]*|` +
					`https?:\/\/[a-z0-9.\-_]{3,63}\.storage\.googleapis\.com\/[^\s"'<>]*|gs:\/\/[a-z0-9.\-_]{3,63}\/[^\s"'<>]*)`),
				[]string{},
				"?",
			},
			{
